...
if ($pdo && isset($_POST['install'])) {
    try {
        ...
        // ── KROK 4: Użytkownicy z formularza ─────────────────
        $usersFromForm = $_POST['users'] ?? [];
        ...
        foreach ($usersFromForm as $u) {
            ...
            if ($uPass !== '') {
                ...
                $messages[] = '&#10003; Konto <strong>' . htmlspecialchars($uLogin) . '</strong> &nbsp;|&nbsp; hasło zapisane';
            } elseif ($exists) {
                // Edycja bez zmiany hasła
                $stmtUupd->execute([$uRole, $uName, $uEmail, $uLogin]);
                $messages[] = '&#10003; Konto <strong>' . htmlspecialchars($uLogin) . '</strong> zaktualizowane (hasło bez zmian)';
            }
            // Nowe konto bez hasła — pomijamy
        }
...
<body>
    <div class="card">
        ...
        <?php else: ?>
            ...
            <form method="POST">
                <div class="box">
                    <strong>Użytkownicy systemu</strong><br>
                    <span style="font-size:12px;color:#6b7280;">Dodaj lub edytuj konta. Zostaw hasło puste, aby nie zmieniać istniejącego (działa tylko przy edycji).</span>
                    ...
                    // Istniejący użytkownicy + 1 pusty wiersz na nowego
                    $formRows = $formUsers;
                    $formRows[] = ['id' => '', 'role_id' => 1, 'name' => '', 'login' => '', 'email' => ''];
                    foreach ($formRows as $ri => $row):
                    ?>
                        <div style="border:1px solid #e5e7eb;border-radius:6px;padding:10px 12px;margin-top:10px;background:#fdfdfd;">
                            <?php if ($row['id']): ?>
                                <div style="font-size:11px;color:#9ca3af;margin-bottom:6px;">Edycja konta #<?= (int)$row['id'] ?></div>
                            <?php else: ?>
                                <div style="font-size:11px;color:#16a34a;margin-bottom:6px;">&#43; Nowe konto</div>
                            <?php endif; ?>

                            <label>Login</label>
                            <input type="text" name="users[<?= $ri ?>][login]"
                                value="<?= htmlspecialchars($row['login'] ?? '') ?>"
                                placeholder="np. jkowalski"
                                style="width:100%;padding:7px 9px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;margin-bottom:6px;">

                            <label>Imię i Nazwisko</label>
                            <input type="text" name="users[<?= $ri ?>][name]"
                                value="<?= htmlspecialchars($row['name'] ?? '') ?>"
                                placeholder="Jan Kowalski"
                                style="width:100%;padding:7px 9px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;margin-bottom:6px;">

                            <label>E-mail</label>
                            <input type="email" name="users[<?= $ri ?>][email]"
                                value="<?= htmlspecialchars($row['email'] ?? '') ?>"
                                placeholder="jan@firma.pl"
                                style="width:100%;padding:7px 9px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;margin-bottom:6px;">

                            <label>Rola</label>
                            <select name="users[<?= $ri ?>][role_id]"
                                style="width:100%;padding:7px 9px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;margin-bottom:6px;">
                                <?php foreach ($formRoles as $r): ?>
                                    <option value="<?= (int)$r['id'] ?>"
                                        <?= (int)($row['role_id'] ?? 1) === (int)$r['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($r['label']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>

                            <label>Hasło <span style="font-weight:400;color:#9ca3af;">(puste = bez zmiany przy edycji)</span></label>
                            <input type="password" name="users[<?= $ri ?>][password]"
                ...
                <button type="submit" class="btn">&#128640; Zapisz i uruchom instalację</button>
            </form>
