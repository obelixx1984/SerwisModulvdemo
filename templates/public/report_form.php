  ...
  <div class="card">
    <div class="card-body">
      ...
      <form method="POST" action="<?= BASE_URL ?>/index.php?route=report_post">
        ...
        <div class="fg">
          <label class="flbl">Osoba zgłaszająca <span class="req">*</span></label>
          <select name="reporter_acronym" class="fc" required>
            <option value="">— Wybierz —</option>
            <?php
            $selectedReporter = $_POST['reporter_acronym']
                ?? $_GET['reporter'] ?? '';
            ?>
            <?php foreach ($employees as $e): ?>
            <option value="<?= Helpers::e($e['acronym']) ?>"
              <?= $selectedReporter === $e['acronym'] ? 'selected' : '' ?>>
              <?= Helpers::e($e['acronym']) ?> – <?= Helpers::e($e['name']) ?>
            </option>
            <?php endforeach; ?>
          </select>
        </div>
      ...
      <p style="text-align:center;margin-top:10px;font-size:12px;color:#6b7280;">
        <?php if (Auth::check()): ?>
          <a href="<?= BASE_URL ?>/index.php?route=dashboard" style="color:#0a2463;">← Wróć do panelu</a>
        <?php else: ?>
          <a href="<?= BASE_URL ?>/index.php?route=login" style="color:#0a2463;">Masz konto? Zaloguj się →</a>
        <?php endif; ?>
      </p>
    </div>
  </div>
...
(function() {
    ...
    dictSel.addEventListener('change', function() {
        ...
        if (!dw || !dt) return;
        if (!dictId || !lineId) { dw.style.display='none'; return; }
