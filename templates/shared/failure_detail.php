    ...
    <div class="card mb2">
      ...
      <div class="card-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px 20px;">
          ...
          <div><div class="flbl">Objaw zgłoszony</div><div class="fw6"><?= Helpers::e($failure['symptom_name'] ?? '—') ?></div></div>
          <div><div class="flbl">Zgłaszający</div><div><?= Helpers::e($failure['reporter_name'] ?? $failure['reporter_acronym'] ?? '—') ?></div></div>
          <div><div class="flbl">Rodzaj awarii</div><div><?= $failure['category_id'] ? Helpers::catBadge($failure['cat_label'], $failure['cat_color']) : '<span class="muted fs-sm">Brak — uzupełni mechanik</span>' ?></div></div>
          ...
          <?php endif; ?>
          <div style="grid-column:1/-1;"><div class="flbl">Usterka</div><div class="fw6">
          ...
          <div style="grid-column:1/-1;"><div class="flbl">Opis dodatkowy</div><div><?= nl2br(Helpers::e($failure['description'])) ?></div></div>
          <?php endif; ?>
          <?php if ($failure['other_failure'] && $failure['mechanic_note']): ?>
          <div style="grid-column:1/-1;"><div class="flbl">Notatka mechanika</div><div><?= nl2br(Helpers::e($failure['mechanic_note'])) ?></div></div>
          <?php endif; ?>
        </div>
      </div>
    </div>
