...
<div class="card mb2" style="padding:10px 16px;display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end;">
  <form method="GET" action="<?= BASE_URL ?>/index.php" style="display:contents;">
    ...
    <div class="fg" style="margin:0;flex:1;min-width:140px;"><label class="flbl">Linia</label>
      <select name="line_id" class="fc">
        <option value="">Wszystkie linie</option>
        <?php foreach ($lines as $l): ?>
        <option value="<?= $l['id'] ?>" <?= ($_GET['line_id']??'')==$l['id']?'selected':'' ?>><?= Helpers::e($l['name']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <div class="fg" style="margin:0;flex:1;min-width:120px;"><label class="flbl">Status</label>
...
<div class="dur-card">
  <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:6px;">
    <div>
      <div class="dur-title"><?= Helpers::reviewTypeLabel($r['review_type']) ?> — <?= Helpers::e($r['line_name']) ?>
        <?= $r['subsystem_name'] ? ' · '.Helpers::e($r['subsystem_name']) : '' ?></div>
      <div class="dur-meta"><?= Helpers::e($r['review_date']) ?> · <?= Helpers::e($r['performer_name']) ?>
        <?= $r['duration_minutes'] ? ' · '.(int)$r['duration_minutes'].' min' : '' ?></div>
    </div>
  ...
  <?php if ($r['notes']): ?>
  <div style="margin-top:6px;padding:5px 8px;background:#fffbeb;border-radius:5px;font-size:12px;color:#78350f;">⚠ <?= Helpers::e($r['notes']) ?></div>
  <?php endif; ?>
...
<div style="grid-column:1/-1;" class="card">
  <div class="card-body" style="text-align:center;padding:24px;" class="muted">Brak raportów DUR.</div>
</div>
<?php endif; ?>
</div>
