              ...
              <tr>
                ...
                <td class="fs-sm muted">
                  <?php if ($l['subsystems_str']): ?>
                    <?php
                    // Rozbij string na tablicę podzespołów
                    $subsArr  = explode('|||', $l['subsystems_str']);
                    // Pełna lista do tooltipa
                    $subsText = implode(', ', $subsArr);
                    // Liczba podzespołów
                    $count    = count($subsArr);
                    // Pokaż tylko 3 pierwsze
                    $preview  = implode(', ', array_slice($subsArr, 0, 3));
                    // Jeśli więcej niż 3 — dodaj badge z licznikiem
                    $more     = $count > 3
                      ? ' <span class="badge" style="background:#e5e7eb;color:#374151;">+' . ($count - 3) . ' więcej</span>'
                      : '';
                    ?>
                    <span title="<?= Helpers::e($subsText) ?>" style="cursor:help;">
                      <?= Helpers::e($preview) ?><?= $more ?>
                    </span>
                  <?php else: ?>
...
<script>
  ...
  function resetLineForm() {
    ...
    ['lName', 'lPrefix', 'lDesc', 'lSubsystems'].forEach(function(id) {
