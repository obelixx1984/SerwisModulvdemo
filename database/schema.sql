...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `nickname`      VARCHAR(80)  NOT NULL COMMENT 'Używany do logowania zamiast e-mail',
...
-- ────────────────────────────────────────────────────────────
-- pracownicy z akronimami (do wyboru w formularzu zgłoszenia)
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `employees` (
  `id`         INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `acronym`    VARCHAR(10)  NOT NULL COMMENT 'Akronim, np. JKO',
  `name`       VARCHAR(150) NOT NULL,
  ...
  `updated_at` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_emp_acronym` (`acronym`),
...
CREATE TABLE IF NOT EXISTS `maintenance_templates` (
  `id`          INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name`        VARCHAR(150) NOT NULL,
  `review_type` ENUM('weekly','monthly','quarterly','biannual','annual','ad_hoc') NOT NULL DEFAULT 'monthly',
  `description` TEXT NULL,
...
CREATE TABLE IF NOT EXISTS `maintenance_schedules` (
  ...
  `review_type`        ENUM('weekly','monthly','quarterly','biannual','annual','ad_hoc') NOT NULL,
